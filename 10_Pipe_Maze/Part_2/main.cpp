...
int main(){
    ...
    std::vector<std::vector<Pipe>> grid(nrows, std::vector<Pipe>(ncols, Pipe(nullptr, ' ', std::make_pair(-1, -1), 0))); // 2D vector with all the grid
    
    // Save the input in a grid
    std::pair<int, int> initialPosition = std::make_pair(-1, -1);
    Pipe* startingPipe = new Pipe(nullptr, 'S', initialPosition, 0);
    fillGrid(grid, input, *startingPipe);

    // Call the algorithm
    int biggestDistance = pipeMaze(startingPipe, grid);

    // See how does the loop look like
